...
export default {
  ...
  methods: {
    async click(mode) {
      const response = await postData(`https://maple.bluesparc.net:8443/api/inputs/${this.which}`, {value: mode});
  ...
  setup(props) {
    ...
    onMounted(async () => {
      var es = new EventSource(`https://maple.bluesparc.net:8443/api/inputs/${props.which}`);
      es.onmessage = (event) => {
